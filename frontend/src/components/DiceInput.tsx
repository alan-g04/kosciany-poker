...
export function DiceInput() {
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          {dice.map((face, idx) => (
            <div key={idx} className="flex flex-col items-center gap-2">
              <Die face={face} onClick={() => cycleDie(idx)} active={firstRoll} />
  ...
  // ---- Virtual dice mode (3 rolls + keep mechanic) ----
          ...
          return (
            <div key={idx} className="flex flex-col items-center gap-2">
              <div className={`relative ${kept ? 'ring-2 ring-emerald-400/80 rounded-xl' : ''}`}>
                <Die
                  face={face}
                  onClick={canKeep ? () => toggleKept(idx) : undefined}
                  active={firstRoll}
                  rollKey={dieNonces[idx]}
                />
                {kept && (
